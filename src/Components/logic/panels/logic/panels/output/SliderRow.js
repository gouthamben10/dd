  ...
  render() {
    ...
    if (device == "*SNI#") {
      ...
      for (let i = 0; i < connect.length; i++) {
        if (PortConnections[connect[i]]) {
          ...
          if(port=="MOTOR1"){defaultport="M1"}
          else if(port=="MOTOR2"){defaultport="M2"}
          else{ defaultport = port;}
    ...
    return (
      <tr className="hide" style={outputStyle}>
        <td style={{ padding: '1%', textAlign: "start", fontWeight: 'bold' }}>
          <Checkbox checked={assign || false} onChange={(value) => onChange('assign' + port, value)} label={name} />
        </td>
        <span style={{ color: "black", display: "inline-block", position: 'relative', top: '25px', left: "-15px" }}>{defaultport}</span>
