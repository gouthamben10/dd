...
class HexBoard extends Component {
  ...
  simulateProgram = (time,newArray) => {
    console.log("simulateProgram...", this.myRef);
  ...
  render() {
    ...
    console.log(this.props, "==================> HexBoard ");
let newId =0;
    return (
            ...
            >
              {row.map((cell, cIndex) => {
                ...
                if(JSON.stringify(rIndex).charAt(0) == 1){
                  if(cell.type !== 'blank' && (cell.type === 'hardware' || cell.type === 'wait')){
                ...
                return (
                  <Hexagon
                    ...
                    keys={cell.type === "blank" ? null:  rIndex === 1 ? rIndex+""+newId++: rIndex + ""+ cIndex}
                    id={rIndex}
                    ref={this.myRef}
                  />
                );
