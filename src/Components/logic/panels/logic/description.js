...
class Logic extends Component {
        ...
        for (let i = 0; i < keyTypeArray.length; i++) {
          if (keyValue) {
            if (keyValue[1] == keyTypeArray[i]) {
              console.log("OutputValue1====> 2", key, value);
  ...
  render = () => {
    ...
    this.state.tutorialDesc.map((value, index) => {
      console.log("program check khushboo", value, program[index], countLogic);
      ...
      if (countLogic == 0) {
        if (program[index]) {
          ...
          countLogic++;
          console.log("program check value.type ", value.type, program[index]);
          tempData =
            value.type == "hardware" ||
            value.type == "variable" ||
            value.type == "wait"
