...
const Button = React.createClass({
  render: function () {
    ...
    return (
      <div onClick={this.props.onClick} style={Panelstyle}>
        <div
          style={{
...
var RightPanel = React.createClass({
  render: function () {
    ...
    var startState = null;
    if (current === "output" || current === "if")
      startState = this.props.startState;
    return (
      <div
        className="completePanel"
        style={{
          position: "fixed",
          right: up - 35,
          bottom: 0,
      ...
      >
        ...
        >
          <Panel
            workspaceif={this.props.workspace}
            value={value}
            state={state}
            onChange={onChange}
            current={current}
